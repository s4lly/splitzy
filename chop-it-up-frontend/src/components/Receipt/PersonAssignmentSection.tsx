...
const PersonAssignmentSection: React.FC<PersonAssignmentSectionProps> = ({
  ...
  const classes = isMobile ? "mt-2 pt-2 border-t border-border/20" : "flex flex-wrap gap-1 justify-center";
    ...
    <div className={`${classes} ${className}`}>
      {isMobile && (
        <div className="flex justify-between items-center mb-1">
          ...
          {item.assignments.length === 0 && people.length > 0 && (
      ...
      {people.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {item.assignments.map((person, personIdx) => (
            <div
              key={personIdx}
              className={clsx(isMobile && "flex items-center gap-1 bg-muted/20 rounded py-1 px-2 group")}
            >
              <PersonBadge
                name={person}
