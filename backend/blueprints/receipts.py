...
def get_user_receipts():
    ...
    try:
        user_receipts = (
            UserReceipt.query.filter_by(user_id=current_user.id)
            .order_by(UserReceipt.created_at.desc())
            .all()
        )
        ...
        for receipt in user_receipts:
            try:
                ...
                current_app.logger.error(
                    f"Error processing receipt ID {receipt.id}: {str(receipt_error)}"
                )
        ...
        return jsonify(
            {"success": False, "error": "Failed to fetch receipt history"}
        ), 500
...
def get_user_receipt(receipt_id):
    ...
    try:
        ...
        return jsonify({"success": True, "receipt": response_receipt})
...
def get_receipt_image(receipt_id):
    ...
    try:
        ...
        if not os.path.isfile(resolved_path):
            return jsonify(
                {"success": False, "error": "Receipt image file not found on server"}
            ), 404
        ...
        current_app.logger.error(
            f"Error retrieving receipt image for receipt ID {receipt_id}: {str(e)}"
        )
        return jsonify(
            {"success": False, "error": "Failed to retrieve receipt image"}
        ), 500
...
def update_receipt_assignments(receipt_id):
    ...
    if not data or "line_item_id" not in data or "assignments" not in data:
        return jsonify(
            {"success": False, "error": "Missing line_item_id or assignments data"}
        ), 400
    ...
    try:
        ...
        line_item = ReceiptLineItem.query.filter_by(
            id=line_item_id, receipt_id=receipt_id
        ).first()
        ...
        db.session.rollback()
        current_app.logger.error(
            f"Error updating assignments for line item {line_item_id}: {str(e)}"
        )
        return jsonify({"success": False, "error": "Failed to update assignments"}), 500


@receipts_bp.route(
    "/api/user/receipts/<int:receipt_id>/line-items/<item_id>", methods=["PUT"]
)
def update_line_item(receipt_id, item_id):
    ...
    try:
        ...
        line_item = ReceiptLineItem.query.filter_by(
            id=item_id, receipt_id=receipt_id
        ).first()
...
def update_receipt_data(receipt_id):
    ...
    try:
        ...
        for key, value in data.items():
            ...
            if key not in ["line_items", "id", "user_id", "created_at"] and hasattr(
                ...
                if key in ["line_items", "id", "user_id", "created_at"]:
                    ...
                    current_app.logger.warning(
                        f"[update_receipt_data] Invalid field '{key}' for receipt"
                    )
        ...
        db.session.rollback()
        current_app.logger.error(
            f"[update_receipt_data] Error for receipt ID {receipt_id}: {str(e)}"
        )
        return jsonify(
            {"success": False, "error": "Failed to update receipt data"}
        ), 500


@receipts_bp.route(
    "/api/user/receipts/<int:receipt_id>/line-items/<item_id>", methods=["DELETE"]
)
...
def get_line_items(receipt_id):
    ...
    try:
        ...
        return jsonify({"success": True, "line_items": items})

    except Exception as e:
        current_app.logger.error(
            f"Error fetching line items for receipt ID {receipt_id}: {str(e)}"
        )
...
def add_line_item(receipt_id):
    ...
    try:
        ...
        return jsonify({"success": True, "line_item": line_item_response.model_dump()})
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(
            f"Error adding line item to receipt ID {receipt_id}: {str(e)}"
        )
